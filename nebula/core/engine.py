...
class Engine:
    ...
    async def _control_leadership_transfer_callback(self, source, message):
        ...
        if await self._round_in_process_lock.locked_async():
            ...
            try:
                ...
                logging.info("DEBUG, LOCK: Just before calling wait_for on the asyncio task to acquire the _round_in_process_lock.")
    ...
    async def _federation_federation_models_included_callback(self, source, message):
        ...
        try:
            logging.info("DEBUG, LOCK: Just before calling acquire_async to acquire the get_connections_lock.")
    ...
    async def update_model_learning_rate(self, new_lr):
        ...
        """
        logging.info("DEBUG, LOCK: Just before calling acquire_async to acquire the trainning_in_progress_lock.")
    ...
    async def _start_learning_late(self):
        ...
        logging.info("DEBUG, LOCK: Just before calling acquire_async to acquire the learning_cycle_lock.")
        await self.learning_cycle_lock.acquire_async()
        try:
            model_serialized, rounds, round, _epochs = await self.sa.get_trainning_info()
            self.total_rounds = rounds
            epochs = _epochs
            logging.info("DEBUG, LOCK: Just before calling acquire_async to acquire the get_round_lock.")
    ...
    async def deploy_federation(self):
        ...
        """
        logging.info("DEBUG, LOCK: Just before calling acquire_async to acquire the federation_ready_lock.")
    ...
    async def _start_learning(self):
        ...
        try:
            if self.round is None:
                ...
                logging.info("DEBUG, LOCK: Just before calling acquire_async to acquire the get_federation_ready_lock.")
